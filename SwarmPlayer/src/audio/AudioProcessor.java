...
public class AudioProcessor {
	...
	public void findOnsets(double p1, double p2){
		...
		for (int i = 0; i < onsets.length; i++) {
			...
			Event e = BeatTrack.newBeat(onsets[i], 0);
//			if (debug)
...
//			e.salience = slope[index];	// or combination of energy + slope??
	...
	/**
	 * Function to filter list of onsets to only most salient onsets
	 * @param percent Percentage of beats to be left in the list
	 * @return EventList of onsets with top 'percent' of beats
	 */
	public EventList filterMajorOnsets(int percent)
	{
		EventList majorOnsetList = new EventList(onsetList);
		
		majorOnsetList.sort(new Comparator<Event>()
				{
					@Override
					public int compare(Event o1, Event o2) 
					{
						if(o1.salience < o2.salience)
						{
							return -1;
						}
						else if(o1.salience > o2.salience)
						{
							return 1;
						}
						else 
		...
		int top = (int) Math.round((double)(majorOnsetList.size() * (100 - percent)) / 100);
		...
		majorOnsetList.sort(new Comparator<Event>()
				{
					...
					public int compare(Event o1, Event o2) 
					{
						if(o1.pedalUp < o2.pedalUp)
						{
							return -1;
						}
						else if(o1.pedalUp > o2.pedalUp)
						{
							return 1;
						}
						else 
		...
		return majorOnsetList;
	}
	
	/**
	 * Function to get the tempo of audio file
	 * @return Tempo value as an int
	 */
	public int getTempo()
	{
		return this.tempo;
	...
	/**
	 * Function to set the tempo value in the processor
	 * @param tempo Tempo value to be set as an int
	 */
	...
	/**
	 * Function to get a list of all beats identified by beat tracking
	 * @return EventList of all onsets
	 */
